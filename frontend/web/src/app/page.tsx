  ...
  return (
    <div className="flex min-h-screen flex-col bg-white">
      ...
      <main className="flex flex-1 flex-col items-center justify-center px-4 text-center">
        <div className="max-w-2xl space-y-6">
          <div className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-4 py-1.5 text-sm font-medium text-emerald-700 ring-1 ring-emerald-200">
            ...
            HACKATHON&apos;26 Projesi
          </div>

          <h1 className="text-5xl font-black tracking-tight text-zinc-900 md:text-6xl">
            Almadan önce <span className="text-emerald-500">sorgula.</span>
          </h1>

          <p className="text-lg text-zinc-500 leading-relaxed">
            SepetIQ, &quot;sepete ekle&quot; butonuna basmadan önce devreye
            girer. Ürünü önermez, almak istediğin ürünü gerçekten alıp almaman
        ...
        <div className="mt-16 flex flex-wrap justify-center gap-4">
          {[
            {
              icon: ShieldCheck,
              label: "3 Skor Analizi",
              desc: "Product Fit · Review Risk · Need Score",
            },
            {
              icon: BarChart3,
              label: "7 LLM Ajan",
              desc: "Cyclic Intelligence akışı",
            },
            {
              icon: Puzzle,
              label: "Browser Eklentisi",
              desc: "Gerçek e-ticaret sayfalarında çalışır",
            },
          ].map(({ icon: Icon, label, desc }) => (
            <div
              key={label}
              className="flex items-start gap-3 rounded-xl border bg-zinc-50 p-4 text-left w-56"
            >
      ...
      <footer className="border-t py-6 text-center text-xs text-zinc-400">
        SepetIQ — Bilinçli alışveriş için tasarlandı.
      </footer>
    </div>
  );
}
