...
const MODES: { id: ShoppingMode; label: string; icon: typeof ShieldCheck; description: string; color: string }[] = [
  {
    id: 'soft',
    label: 'Soft (Yumuşak)',
    icon: ShieldCheck,
...
const modeStyles: Record<string, { border: string; bg: string; ring: string; iconBg: string; activeIconBg: string }> = {
  emerald: {
  ...
  return (
    <div className="max-w-4xl space-y-8">
      <div>
        ...
        <p className="text-muted-foreground">SepetIQ&apos;nun size nasıl davranacağını belirleyin.</p>
      </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {MODES.map((mode) => {
            ...
            return (
              <button
                key={mode.id}
                onClick={() => setPrefs(prev => ({ ...prev, default_mode: mode.id }))}
                className={cn(
              ...
              >
                <div className={cn(
                  "mb-4 p-2 rounded-lg",
                  isActive ? `${style.activeIconBg} text-white` : "bg-zinc-100 text-zinc-400"
                )}>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Limitlerinizi belirleyerek kontrolü elinizde tutun.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="relative">
              <Input
                ...
                onChange={e => setPrefs(prev => ({ ...prev, monthly_budget: parseFloat(e.target.value) }))}
              />
            </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>SepetIQ sizi ne zaman bilgilendirsin?</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between p-3 rounded-lg border bg-zinc-50">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-zinc-500">Kritik durumlarda anlık uyarı al.</p>
              </div>
              <button
                onClick={() => setPrefs(prev => ({ ...prev, notifications_enabled: !prev.notifications_enabled }))}
                className={cn(
                  "relative h-6 w-11 rounded-full transition-colors",
                  prefs.notifications_enabled ? "bg-emerald-500" : "bg-zinc-200"
                )}
              >
                <div className={cn(
      ...
      <div className="flex items-center justify-end pt-6 border-t">
        <Button
          onClick={handleSave}
          disabled={loading}
          className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold h-12 px-8 min-w-[200px]"
        >
          {loading ? 'Kaydediliyor...' : success ? (
