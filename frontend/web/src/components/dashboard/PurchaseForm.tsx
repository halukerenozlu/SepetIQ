...
const CATEGORIES = [
  'Elektronik', 'Giyim', 'Kozmetik', 'Ev & Yaşam', 'Spor', 'Kitap', 'Diğer'
];
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <label className="text-sm font-medium text-zinc-700">Ürün Adı</label>
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-zinc-700">Fiyat (₺)</label>
                <div className="relative">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-zinc-700">Alış Tarihi</label>
                <div className="relative">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-zinc-700">Kullanım Sıklığı</label>
                <div className="relative">
                  <Clock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-zinc-400" />
                  <select 
                    className="w-full pl-10 h-10 rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-500"
                    value={formData.usage_frequency}
                    onChange={e => setFormData(prev => ({ ...prev, usage_frequency: e.target.value as UsageFrequency }))}
                  >
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-zinc-700">Memnuniyet</label>
                <div className="relative">
                  <Smile className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-zinc-400" />
                  <select 
                    className="w-full pl-10 h-10 rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-500"
                    value={formData.satisfaction}
                    onChange={e => setFormData(prev => ({ ...prev, satisfaction: e.target.value as Satisfaction }))}
                  >
                    {SATISFACTION.map(s => <option key={s.value} value={s.value}>{s.icon} {s.label}</option>)}
                  </select>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-zinc-700">Notlar (Opsiyonel)</label>
              <textarea 
                className="w-full min-h-[80px] rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-500"
